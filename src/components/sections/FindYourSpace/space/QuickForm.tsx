...
const purposeOptions = {
  ...
  options: ["Photoshoot", "Video Shoot", "Reel Creation", "Product Shoot", "Podcast Recording", "Workshop/Class", "Interview", "Private Meetup", "Event"]
}
  ...
  return (
    <div className='max-w-[1500px] w-full mx-auto bg-[#FFFFFF] rounded-2xl border border-[#2C2C2C] flex flex-col xl:flex-row justify-between gap-x-[20px] gap-y-[60px] items-center lg:items-start px-4 sm:px-6 py-2.5'>
      ...
      <form
        ...
        className='w-full lg:flex-1 shadow-[1px_1px_16px_gray] rounded-3xl flex flex-col justify-center items-center px-4 sm:px-5 md:px-6 lg:px-7 pt-5 sm:pt-6 md:pt-7 lg:pt-8 pb-7 sm:pb-8 md:pb-9 lg:pb-10 mt-[20px] mb-[30px]'
      >
        <p className='font-semibold text-[#BA181B] text-[20px] sm:text-[22px] md:text-[23.5px] lg:text-[25px] text-center'>
          Quick Form
        </p>
        <p className='font-light text-[#BA181B] text-[15px] sm:text-base md:text-[18px] lg:text-[20px] mt-[-3px] text-center'>
          One step closer to get your desired space
        </p>
        ...
        <div className='w-full flex flex-col gap-[30px] mt-[29px]'>
          <input
            type="text"
            placeholder='Name*'
            className='w-full bg-[#D9D9D957] placeholder:text-[14px] md:placeholder:text-base placeholder:text-[#00000033] rounded-md py-3 px-4 outline-none'
            value={name}
